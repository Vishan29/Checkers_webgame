...
angular.module('ngCheckers', [])
  ...
  .controller('checkersCtrl', function($scope, $timeout) { 
    var RED = "Red", BROWN = "Black", BOARD_WIDTH = 8,
        selectedSquare = null;
      ...
      for (var i = 0; i < BOARD_WIDTH; i++) {
        ...
        for (var j = 0; j < BOARD_WIDTH; j++) {
          if ( (i === 0 &&  j % 2 === 0) || (i === 1 && j % 2 === 1) ) {
            $scope.board[i][j] = new Piece(BROWN, j, i);
    ...
    function movePiece(square) {
      if (square.isChoice) {
        ...
        $scope.player = $scope.player === RED ? BROWN : RED;
      }
    }
    ...
    function becomeKingAfterJump(x, y){
      return ($scope.player === RED && y == 1) ||
             ($scope.player === BROWN && y == BOARD_WIDTH - 2);
    }
    ...
    function setChoices(x, y, depth, matados, oldX, oldY, isKing) {
      ...
      isKing = 
          ...
          ($scope.player === BROWN && y == BOARD_WIDTH - 1);
      // Upper Choices
